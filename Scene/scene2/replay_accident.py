...
BUFFER_TIME = 1.3  # 事故后的缓冲时间
EXCLUDED_VEHICLE_ID = 'routeFlow_4.2'
WEATHER_CHANGE_INTERVAL = 40 # 每40个仿真步骤更改一次天气
...
def determine_time_window(vehicle_file, accident, accident_center):
    ...
    if min_start_time == float('inf'):
        min_start_time = accident_time - 10.0  # 默认回溯10秒
...
def replay_accident(vehicle_data,start_time, end_time,synchronization,accident,carla_simulation ):
    ...
    # 初始化天气参数（清晨有雾）
    fog_density = 0.0  # 初始雾浓度为0
    weather_params = carla.WeatherParameters(
        cloudiness=20.0,  # 少量云，模拟清晨
        precipitation=0.0,  # 无降雨
        precipitation_deposits=0.0,  # 无水坑
        wind_intensity=10.0,  # 轻微风
        sun_azimuth_angle=90.0,  # 太阳方位角，模拟东方升起的清晨
        sun_altitude_angle=10.0,  # 低太阳高度角，模拟清晨
        fog_density=fog_density,  # 初始雾浓度
        fog_distance=10,  # 雾开始距离
        wetness=0.0,  # 无湿度
        fog_falloff=0.1,  # 雾密度适中，类似空气
        scattering_intensity=0.0,  # 无灯光散射
        mie_scattering_scale=0.0,  # 无大颗粒散射
        rayleigh_scattering_scale=0.0331,  # 默认瑞利散射，模拟蓝天
        dust_storm=0.0  # 无沙尘暴
    )
    world.set_weather(weather_params)
    print(f"初始天气设置：清晨，雾浓度={fog_density}")
    ...
    while current_time <= end_time:
        ...
        step_count += 1
        if step_count % WEATHER_CHANGE_INTERVAL == 0 and fog_density < 50.0:
            fog_density = min(fog_density + 10.0, 50.0)  # 增加10，直到50
            weather_params.fog_density = fog_density
            world.set_weather(weather_params)
            print(f"步骤 {step_count}，更新天气：雾浓度={fog_density}")
        # 处理所有车辆
        for vehicle_id, trajectory in vehicle_data.items():
            if vehicle_id == EXCLUDED_VEHICLE_ID:
                continue
            # 如果车辆尚未创建
            ...
            if step_count >= 2:
                ...
                if carla_vehicle:
                    ...
                    offset = carla.Location(z=60)
                    spectator = world.get_spectator()
                    spectator.set_transform(carla.Transform(location + offset, carla.Rotation(pitch=-90, yaw=-107.5)))
        current_time += SIMULATION_STEP

def main():
    try:
        argparser = argparse.ArgumentParser(description=__doc__)
        argparser.add_argument('--sumo_cfg_file', type=str, help='sumo configuration file', default="replay_accident_rou/simulation.sumocfg")
        ...
        argparser.add_argument('--vehicle-trace', type=str, help='车辆轨迹CSV文件',default="sumo_data/vehicle_trace_20250626-192259.csv")
        argparser.add_argument('--collision-trace', type=str, help='事故记录CSV文件',default="sumo_data/collisions_20250626-192259.csv")
        argparser.add_argument('--sumo-config', type=str, help='SUMO配置文件路径', default="simulation.sumocfg")
        argparser.add_argument('--collisions_number', type=int, help='发生的碰撞事故', default="11")

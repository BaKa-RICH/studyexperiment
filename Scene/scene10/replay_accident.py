...
def main():
    vehicle_trace = "sumo_data/vehicle_trace_20251205-154251.csv"
    collisions_trace = "sumo_data/collisions_20251205-154251.csv"
    collisions_number = 0
    try:

        # 初始化联合仿真
        sumo_simulation = SumoSimulation("SuZhou_replay/SuzhouNorthStation.sumocfg", step_length, sumo_host,sumo_port, sumo_gui, client_order)
        ...
        # 步骤1: 加载事故数据
        accidents = load_accident_data(collisions_trace)
        print(len(accidents))
        ...
        # 使用第一个事故
        accident = accidents[collisions_number]
        print(f"复现事故发生在 {accident['timestamp']:.2f}秒")
        print(f"涉及车辆: {accident['vehicle1']} 和 {accident['vehicle2']}")

        # 步骤2: 动态确定事故中心点和时间窗口
        accident_center = get_accident_center(vehicle_trace, accident)
        if not accident_center:
            print("无法确定事故中心点")


        start_time, end_time = determine_time_window(vehicle_trace, accident, accident_center)
        print(f"复现时间窗口: {start_time:.2f}s 到 {end_time:.2f}s")

        # 步骤3: 筛选相关车辆
        vehicle_data = filter_vehicles(vehicle_trace, accident, accident_center, start_time, end_time)

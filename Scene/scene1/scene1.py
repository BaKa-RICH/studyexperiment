...
class SumoDataRecorder:
    ...
    #             collision_key = (collider, victim, collision.type)
    #             if collision_key not in self.recorded_collisions:
    #                 self.recorded_collisions.add(collision_key)
    #                 self.collision_writer.writerow([timestamp, collision_id, collider, victim, collision.type])
    #     except traci.TraCIException:
    #         pass
...
def update_spectator_to_follow_vehicle(world, synchronization, vehicle_id_to_follow):
    ...
    # 4. 计算摄像机的理想位姿
    location = vehicle_transform.location + carla.Location(z=100)
...
def calculate_distance_between_vehicles(vehicle1_id, vehicle2_id):
    """计算两个车辆之间的距离"""
    try:
        pos1 = traci.vehicle.getPosition(vehicle1_id)
        pos2 = traci.vehicle.getPosition(vehicle2_id)
...
def main():
    ...
    argparser.add_argument('--sumo_cfg_file', type=str, help='sumo configuration file', default="scene_1_data_rou/scene1.sumocfg")
    ...
    try:
        while True:
            ...
            if not initial_setup_done and timestamp > 0:
                ...
                # 确保我们的主角车辆已经出现
                if lead_vehicle_id in all_vehicles and subject_vehicle_id in all_vehicles and ambush_vehicle_id in all_vehicles:
                    ...
                    traci.vehicle.setColor(ambush_vehicle_id, (255, 255, 0, 255))    # 侧车黄色
            ...
            # traci.vehicle.setLaneChangeMode(subject_vehicle_id, 1023)
            # traci.vehicle.setSpeedMode(ambush_vehicle_id, 0)
            # traci.vehicle.setLaneChangeMode(ambush_vehicle_id, 1023)

            all_vehicles = traci.vehicle.getIDList()
            
            # --- 在第3秒，触发大雨天气 ---
            if timestamp > 3 and not weather_triggered:
                print(f"\n--- {timestamp:.2f}s: 触发大雨天气，车道线识别失效... ---")
                # 设置极端大雨天气
            ...
            if timestamp > 5 and not event_started:
                if subject_vehicle_id in all_vehicles and ambush_vehicle_id in all_vehicles:
                    print(f"\n--- {timestamp:.2f}s: 车辆因无法识别车道线开始减速行驶... ---")

                    traci.vehicle.slowDown(lead_vehicle_id, 15, 50.0)
                    
                    # 主车减速，模拟因视线不清而降低速度
                    traci.vehicle.slowDown(subject_vehicle_id, 18, 12.0)
                    
                    # 侧车也减速
                    traci.vehicle.slowDown(ambush_vehicle_id, 20, 40.0)
            ...
            # --- 在第10秒，主车开始车道偏离 ---
            if timestamp > 8 and not lane_departure_started:
                if subject_vehicle_id in all_vehicles:

                    print(f"\n--- {timestamp:.2f}s: 主车因无法识别车道线开始偏离... ---")

                    traci.vehicle.setSpeedMode(subject_vehicle_id, 0)
                    traci.vehicle.setSpeedMode(ambush_vehicle_id, 0)
                    # traci.vehicle.setAcceleration(subject_vehicle_id, 0, 5)
                    traci.vehicle.setLaneChangeMode(subject_vehicle_id, 0)  # 禁用所有变道逻辑
                    traci.vehicle.setLaneChangeMode(ambush_vehicle_id, 0)

                    traci.vehicle.changeLane(subject_vehicle_id, 2, 4)  


                    
                    lane_departure_started = True

            if timestamp == 12.20:
                        # traci.vehicle.setSpeedMode(subject_vehicle_id, 31)
                        # traci.vehicle.setSpeedMode(ambush_vehicle_id, 31)


                        traci.vehicle.setSpeed(subject_vehicle_id, 0)
                        traci.vehicle.setSpeed(ambush_vehicle_id, 0)

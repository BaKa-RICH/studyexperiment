...
### 4.4.1 coordination 阶段里 `g_pm / g_ms` 的双角色

当前需要明确区分两个阶段：

- **coordination slice**：`m` 仍在 ramp lane，`g_pm / g_ms` 的负值首先表示“未来 merge gap 还没形成”，而不是“当前横向已经发生碰撞风险”
- **merge slice**：`m` 已进入 lane-change 语义，`g_pm / g_ms` 恢复为硬安全证书，必须在规定区间上连续成立

因此：

- 在 coordination 阶段，`g_pm / g_ms` 应优先被解释成 pairwise virtual gap error
- 在 merge 阶段，`g_pm / g_ms` 才被解释成 hard safety certificate

当前实现已经切到 `e_{pm}^{virt} / e_{ms}^{virt}` 双误差 coordination 控制，并在 merge 前增加了 pairwise virtual gap readiness 与 relative-speed readiness。文献对齐与优化路线见 `docs/active_gap_literature_alignment_and_optimization.md`。
...
并且，当前实现已经补充了 paper-aligned 的联合门槛：

- pairwise gap readiness
- relative-speed readiness

当前版本的 merge gate 已显式同时检查：

- pairwise virtual gap readiness
- relative-speed readiness

真正的 merge 提交仍以 hard certificate 为准。
...
- `docs/active_gap_literature_alignment_and_optimization.md`
  负责记录与代表性论文的对比、当前实现与目标方案的差距、以及后续 coordination 控制器的优化方向。
